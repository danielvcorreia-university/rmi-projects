...
public class jClient {
    ...
    public static void main(String[] args) {
        ...
        double iRSensorAngles[] = {0.0, 50.0, -50.0, 180.0};
    ...
    public void determineAction(boolean followBeacon) {
        float w_l = 0.0f;
        float w_r = 0.0f;
        float k   = 9.3f;
        float lin = (lPow + rPow) / 2.0f;

        if(irSensor0 > 5.0f || irSensor1 > 5.0f || irSensor2 > 5.0f) {
            if(dir) 
                { cif.DriveMotors(+0.1,-0.1); return; }
            else 
                { cif.DriveMotors(-0.1,+0.1); return; }
        }
        ...
        if(irSensor2 < 1.0f) 
            { dir = true; }
        else if(irSensor1 < 1.0f)
            { dir = false; }

        if(dir) { 
            w_r = k * (float) Math.toRadians(irSensor2 - 1.93f);  
            lPow = lin - (w_r / 2.0f); 
            rPow = lin + (w_r / 2.0f);
        }
        else {
            w_l = k * (float) Math.toRadians(irSensor1 - 1.93f);
            lPow = lin + (w_l / 2.0f); 
            rPow = lin - (w_l / 2.0f);
        }
